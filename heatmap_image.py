...
for foldername in sorted(os.listdir('attention_image/result')):
    for filename in os.listdir(f'attention_image/result/{foldername}'):
        image_name = filename.split('.')[0]

        # input_image = Image.open(f'attention_image/result/{foldername}/{filename}').convert('L')
        # cm_hot = mpl.cm.get_cmap('hot')
        # im = np.array(input_image)
        # im = cm_hot(im)
        # im = np.uint8(im * 255)
        # im = Image.fromarray(im)
        # im.save(f"attention_image/result/{foldername}/{image_name}-jet.jpg")

        input_image = mpimg.imread(f'attention_image/result/{foldername}/{filename}')
        im = np.dot(input_image[:,:,:3], [0.229, 0.587, 0.114])
        f = plt.figure(figsize=(4, 4), dpi=128)
        plt.axis('off')
        plt.imshow(im, cmap='hot')
        plt.savefig(f'attention_image/result/{foldername}/{image_name}-jet.jpg', dpi=f.dpi)
        plt.close()
        ...
        # input_image.load()
        # numpy_image = np.asarray(input_image, dtype='int32')

        # cmap = plt.get_cmap('jet')
        
        # rgba_image = cmap(numpy_image)
        # rgba_image.savefig(f"attention_image/result{foldername}/{filename.split('.')[0]}-jet.jpg")
        # break
        # Image.fromarray((cmap_image[:, :, :3] * 255).astype(np.uint8)).save(f"attention_image/result/{foldername}/{filename.split('')[0]}-jet.jpg")
