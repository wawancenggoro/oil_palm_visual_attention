...
title_pattern = re.compile(f'^{title_arg}.+.npy')
org_pattern = re.compile(f'^org_{title_arg}.+.npy')
for filename in sorted(os.listdir('attention_image')):
    if title_pattern.match(filename):
        numpy_attention = np.load(f'attention_image/{filename}')
        ...
        for i in range(attention.size()[0]):
            ...
            vutils.save_image(image_local, f'attention_image/result/{title_arg}_{i}.jpg', normalize=True)
    elif org_pattern.match(filename):
        numpy_image = np.load(f'attention_image/{filename}')
        org_image = torch.from_numpy(numpy_image).float().to('cuda:0')
        vutils.save_image(org_image, f'attention_image/result/{title_arg}.jpg', normalize=True)
